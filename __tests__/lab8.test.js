describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // Expect the text value to be "Remove from Cart"
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let cartNum = await page.$eval('#cart-count', (cart) => {return parseInt(cart.innerText);});
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    expect(await page.$eval('#cart-count', (cart) => {return parseInt(cart.innerText);})).toBe(20);
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    let prodItems = await page.$$('product-item');
    expect(await page.$eval('#cart-count', (cart) => {return parseInt(cart.innerText);})).toBe(0);
    for (let prodItem of prodItems) {
      let shadowRoot = await prodItem.getProperty('shadowRoot');
      let button = await shadowRoot.$('button');
      let buttonText = await button.getProperty('innerText');
